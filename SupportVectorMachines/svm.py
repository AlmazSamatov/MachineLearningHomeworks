...
def function_to_optimize(alphas, y, x, kernel):
    ...
    for i in range(len(alphas)):
        for j in range(len(x)):
            values.append(alphas[i] - 0.5 * alphas[i] * alphas[j] * y[i] * y[j] * kernel(x[i], x[j]))
    return min(values)
...
def find_w_b(alphas, y, x, sv_inds, kernel, thresh, C):
    if kernel == linear_kernel:
        temp = np.zeros(shape=(len(alphas),))
        k = -1
        for i in range(len(x)):
            if thresh < alphas[i] < C:
                k = i
        for i in range(len(alphas)):
            temp = np.add(temp, alphas[i] * y[i] * kernel(x[i], x[k]))
        return None, np.subtract(temp, y[k])
    else:
        w = np.zeros(shape=(len(alphas),))
        for i in range(len(alphas)):
            w = np.add(w, alphas[i] * y[i] * kernel(x[i]))
        for i in range(len(x)):
            if thresh < alphas[i] < C:
                b = np.subtract(np.dot(w, kernel(x[i])), y[i])
                return w, b
...
def predict(alphas, y, x, x_test, b, sv_inds, kernel):
    ...
    for i in range(len(predictions)):
        temp = 0
        for j in range(len(x)):
            temp += alphas[j] * y[j] * kernel(x[j], x_test)
        temp -= b
        if temp >= 0:
            predictions[i] = 1
        else:
...
def minimize_and_plot(X, Y, kernel, C, thresh):
    ...
    alpha_constr = LinearConstraint(identity, [0], [C])  # todo write your code here (one-liner)
